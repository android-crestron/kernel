...
static ssize_t _ppmgr_angle_write(unsigned long val)
{
	...
	printk("_ppmgr_angle_write: angle = %lu\n", angle);
    ...
    if (angle > 3) {
        if (angle == 90)
            ...
            printk("you should set 0 or   0 for   0 clockwise");
            printk("               1 or  90 for  90 clockwise");
            printk("               2 or 180 for 180 clockwise");
...
static ssize_t angle_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    ssize_t size;
    char *endp;
	...
	if (ppmgr_device.force_prot) {		
		size = endp - buf;
		printk("not accept external angle setting\n");
        return count;		
	}
...
static ssize_t orientation_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t bypass_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    ssize_t size;
    char *endp;
...
static ssize_t ppscaler_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    ssize_t size;
    char *endp;
...
static ssize_t ppscaler_rect_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t receiver_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
	ssize_t size;
	char *endp;
    if(buf[0]!='0'&&buf[0]!='1') {
...
static ssize_t platform_type_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
	ssize_t size;
	char *endp;
...
static ssize_t _3dmode_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    ssize_t size;
    char *endp;
...
static ssize_t viewmode_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    ssize_t size;
    char *endp;
    unsigned mode = simple_strtoul(buf, &endp, 0);
...
static ssize_t doublemode_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    ssize_t size;
    char *endp;
...
static ssize_t write_view_mode(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    int r;
    char *endp;
...
static ssize_t write_vertical_sample(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    int r;
    char *endp;
...
static ssize_t write_scale_width(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
    int r;
    char *endp;
...
static ssize_t cut_win_store(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
	set_cut_window(buf);
    return strnlen(buf, count);
}
...
static ssize_t mirror_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
	ssize_t size;
	char *endp;
...
static struct class_attribute ppmgr_class_attrs[] = {
    __ATTR(info,			S_IRUGO | S_IWUSR,				     show_ppmgr_info,                  NULL),
    __ATTR(angle,			S_IRUGO | S_IWUSR | S_IWGRP,	          angle_read,           angle_write),
    __ATTR(rect,			S_IRUGO | S_IWUSR,				           rect_read,            rect_write),
    __ATTR(bypass,			S_IRUGO | S_IWUSR,				         bypass_read,          bypass_write),
    __ATTR(disp,			S_IRUGO | S_IWUSR | S_IWGRP,	           disp_read,            disp_write),
    __ATTR(orientation,		S_IRUGO | S_IWUSR,				    orientation_read,     orientation_write),
#ifdef CONFIG_POST_PROCESS_MANAGER_PPSCALER
    __ATTR(ppscaler,		S_IRUGO | S_IWUSR | S_IWGRP,	       ppscaler_read,        ppscaler_write),
    __ATTR(ppscaler_rect,	S_IRUGO | S_IWUSR | S_IWGRP,	  ppscaler_rect_read,   ppscaler_rect_write),
#endif
       __ATTR(vtarget,		S_IRUGO | S_IWUSR | S_IWGRP,	       receiver_read,        receiver_write),
#ifdef CONFIG_POST_PROCESS_MANAGER_3D_PROCESS
    __ATTR(ppmgr_3d_mode,	S_IRUGO | S_IWUSR,				        _3dmode_read,         _3dmode_write),
    __ATTR(viewmode,		S_IRUGO | S_IWUSR,				       viewmode_read,        viewmode_write),
    __ATTR(doublemode,		S_IRUGO | S_IWUSR,				     doublemode_read,      doublemode_write),
    __ATTR(switchmode,		S_IRUGO | S_IWUSR,				     switchmode_read,      switchmode_write),
    __ATTR(direction_3d,	S_IRUGO | S_IWUSR,				   direction_3d_read,    direction_3d_write),
    __ATTR(scale_down,		S_IRUGO | S_IWUSR,				     scale_down_read,      scale_down_write),
    __ATTR(depth,			S_IRUGO | S_IWUSR,				          read_depth,           write_depth),
    __ATTR(view_mode,		S_IRUGO | S_IWUSR,				      read_view_mode,       write_view_mode),
    __ATTR(vertical_sample,	S_IRUGO | S_IWUSR,				read_vertical_sample, write_vertical_sample),
    __ATTR(scale_width,		S_IRUGO | S_IWUSR,				    read_scale_width,     write_scale_width),
    __ATTR(axis,			S_IRUGO | S_IWUSR,				        cut_win_show,         cut_win_store),
#endif
    __ATTR(platform_type,	S_IRUGO | S_IWUSR,				  platform_type_read,   platform_type_write),
    __ATTR(mirror,			S_IRUGO | S_IWUSR,				         mirror_read,          mirror_write),
    __ATTR_RO(												 ppmgr_vframe_states                       ),
    __ATTR(disable_prot,	S_IRUGO | S_IWUSR,				   disable_prot_show,    disable_prot_store),
    __ATTR_NULL
};
...
int  init_ppmgr_device(void)
{
	...
	pLcdConf = get_lcd_config();
	printk("LCD Model Name is: %s\n", pLcdConf->lcd_basic.model_name);
	if (strncmp(pLcdConf->lcd_basic.model_name, "TFT540960", 9) == 0)
	{
     	ppmgr_device.force_prot  = 1;
		ppmgr_device.orientation = 1;
		printk("Device width is 540\n");
	}
	else
	{
		ppmgr_device.orientation = 0;
		ppmgr_device.force_prot  = 0;
		printk("Device width is NOT 540\n");
	}
    ...
    if((ppmgr_device.cla = init_ppmgr_cls()) == NULL)
		return -1;
...
static int __init ppmgr_init_module(void)
{
    int err;
...
static void __exit ppmgr_remove_module(void)
{

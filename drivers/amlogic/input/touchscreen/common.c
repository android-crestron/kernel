...
GET_DT_ERR_TYPE get_dt_data(struct device_node* of_node, struct touch_pdata *pdata)
{
	...
	if (pdata->fw_file)
		strcpy(pdata->truly_std_fw_file, "/system/etc/touch/touch.fw");
	if (pdata->fw_file)
		strcpy(pdata->truly_gov_fw_file, "/system/etc/touch/touch.fw");
	if (pdata->fw_file)
		strcpy(pdata->edt_std_fw_file, "/system/etc/touch/touch.fw");
	if (pdata->fw_file)
		strcpy(pdata->edt_gov_fw_file, "/system/etc/touch/touch.fw");
	...
	if (err) {
		printk("%s info: get max num, set max num 10!\n", pdata->owner);
		pdata->max_num = 10;
	}
	...
	err = of_property_read_string(of_node,"truly_std_fw_file",&str);
	if (err) {
		printk("%s info: get truly_std_fw_file, set firmware %s!\n",pdata->owner, pdata->truly_std_fw_file);
	}
	else {
		strcpy(pdata->truly_std_fw_file, str);
		printk("%s get truly_std_fw_file, set firmware %s!\n",pdata->owner, pdata->truly_std_fw_file);
	}

	err = of_property_read_string(of_node,"truly_gov_fw_file",&str);
	if (err) {
		printk("%s info: get truly_gov_fw_file, set firmware %s!\n",pdata->owner, pdata->truly_gov_fw_file);
	}
	else {
		strcpy(pdata->truly_gov_fw_file, str);
		printk("%s get truly_gov_fw_file, set firmware %s!\n",pdata->owner, pdata->truly_gov_fw_file);
	}

	err = of_property_read_string(of_node,"edt_std_fw_file",&str);
	if (err) {
		printk("%s info: get edt_std_fw_file, set firmware %s!\n",pdata->owner, pdata->edt_std_fw_file);
	}
	else {
		strcpy(pdata->edt_std_fw_file, str);
		printk("%s get edt_std_fw_file, set firmware %s!\n",pdata->owner, pdata->edt_std_fw_file);
	}

	err = of_property_read_string(of_node,"edt_gov_fw_file",&str);
	if (err) {
		printk("%s info: get edt_gov_fw_file, set firmware %s!\n",pdata->owner, pdata->edt_gov_fw_file);
	}
	else {
		strcpy(pdata->edt_gov_fw_file, str);
		printk("%s get edt_gov_fw_file, set firmware %s!\n",pdata->owner, pdata->edt_gov_fw_file);
	}
...
static ssize_t touch_class_write(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strcmp(attr->attr.name, "PrintkFlag")) {
		...
		if(ts_com->read_version)
			ts_com->read_version(NULL);
	} else if (!strcmp(attr->attr.name, "UpgradeFWWithAPP")) {
		if(ts_com->upgrade_fw_with_app) {
			disable_irq_nosync(ts_com->irq);
			memset(fwname, 0, sizeof(fwname));
			sprintf(fwname, "%s", buf);
			fwname[count-1] = '\0';
			printk("fwname = %s\n", fwname);
			ts_com->upgrade_fw_with_app(fwname);
			enable_irq(ts_com->irq);
		}
	} else if (!strcmp(attr->attr.name, "upgrade")) {
		if(ts_com->upgrade_touch) {
...
static struct class_attribute touch_class_attrs[] = {
    ...
    __ATTR(UpgradeFWWithAPP, S_IRUGO | S_IWUSR, touch_class_read, touch_class_write),
